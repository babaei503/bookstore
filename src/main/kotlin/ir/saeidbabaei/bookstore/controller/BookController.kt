...
class BookController(private val bookService: BookService) {
    ...
    @ApiResponses(
        value = [ApiResponse(code = 200, message = "OK", response = Book::class),
    ...
    @ApiResponses(
        value = [ApiResponse(code = 201, message = "Created", response = Book::class),
                 ApiResponse(code = 409, message = "Book didn't save - Duplicate book", response = ApiError::class),
	...
	fun createBook(@Valid @RequestBody book: Book, uri: UriComponentsBuilder): ResponseEntity<Book> {
        ...
        headers.location = uri.path("/api/books/{uuid}").buildAndExpand(newBook.bookId).toUri()
